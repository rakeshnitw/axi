  ...
  """
    ...
    if not groupName:
        ...
        if len(groupName.split("@")) != 2:
            print(
                "ERROR: Invalid group name. It must be of group@domain form.",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    try:
        ...
        print(
            "Could not open addresses file (%s), or none specified" % addrFile,
            file=sys.stderr,
        )
        show_help()
        sys.exit(1)
    ...
    try:
        ...
        print(
            "ERROR: Could not enter context for domain `%s`" % domain, file=sys.stderr
        )
        sys.exit(2)
    if group not in c.get_forwarders():
        print(
            "Group `%s` does not exist in domain `%s`. Trying to create..."
            % (group, domain)
        )
        try:
            ...
            print(
                "ERROR: Could not add group `%s` in domain `%s`." % (group, domain),
                file=sys.stderr,
            )
            sys.exit(2)
    try:
    ...
    for line in fd:
        ...
        try:
            ...
            print(
                "ERROR: Address `%s` could not be added (%s:%d)"
                % (addr, addrFile, lineNo),
                file=sys.stderr,
            )
