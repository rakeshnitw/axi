...
if __name__ == "__main__":
    ...
    for arg in args:
        if arg[:6].startswith("cli://"):
            ...
            if not "@" in arg[6:]:
                print(
                    "WARNING: Incorrect cli url specification. Using defaults",
                    file=sys.stderr,
                )
                continue
    ...
    if len(args) != 3:
        print(
            "Usage: %s [cliURI] <domain> <account> <new name>" % sys.argv[0],
            file=sys.stderr,
        )
    ...
    if not c.hasAccount(domain, account):
        print(
            "ERROR: Account '%s' does not exist in domain '%s'" % (account, domain),
            file=sys.stderr,
        )
        sys.exit(3)
    try:
        ...
        print(
            "ERROR: Could not rename '%s' as '%s'" % (account, newname), file=sys.stderr
        )
        sys.exit(4)
