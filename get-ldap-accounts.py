...
if __name__ == "__main__":
    ...
    if os.environ.get("LDAP_SPECIAL_ACCOUNTS"):
        specialAccounts = [
            x.strip()
            for x in os.environ.get("LDAP_SPECIAL_ACCOUNTS").split(",")
            if x.strip()
        ]
    specialAccountsMask = []
    if os.environ.get("LDAP_SPECIAL_ACCOUNTS_MASK"):
        specialAccountsMask = [
            x.strip()
            for x in os.environ.get("LDAP_SPECIAL_ACCOUNTS_MASK").split(",")
            if x.strip()
        ]
    ...
    if len(sys.argv) > 1:
        for arg in sys.argv[1:]:
            if arg.lower().startswith("domain="):
                ...
                specialAccountsMask = [
                    x.strip() for x in arg[12:].split(",") if x.strip()
                ]
            elif arg.lower() in ["-h", "/?", "--help"]:
                print("AXIGEN LDAP Query Helper")
                print(
                    "Usage: %s [-h|--help|/?] [domain=<domain>] [-p] [host=<ldap host>] [binduser=<bind user>] [bindpass=<bind password>] [basedn=<base DN>] [filter=<search filter>] [attr=<account attribute>] [exclude=<comma-separated accounts>] [excludemask=<comma-separated regexes>]"
                    % os.path.basename(sys.argv[0])
                )
                print("       -h | --help | /? -> print this help")
                print("       -p               -> print a tab delimited password field")
                print(
                    "                           (password is base64 encoded of a 16 character"
                )
                print("                           random string)")
                print(
                    "       domain=<domain>  -> a @<domain> string will be appended to each"
                )
                print("                           printed user")
                print(
                    "                           (useful for the import-accounts.py script)"
                )
                ...
                print(
                    "       filter=<filter>  -> LDAP search filter (default: (objectClass=user))"
                )
                print(
                    "       attr=<attr>      -> account attribute name (default: sAMAccountName)"
                )
                print(
                    "       exclude=<list>   -> comma-separated list of accounts to exclude"
                )
                print(
                    "       excludemask=<list> -> comma-separated list of regex masks to exclude"
                )
                print(
                    "     Environment variables LDAP_HOST, LDAP_BIND_USER, LDAP_BIND_PASS,"
                )
                print("     LDAP_BASE_DN, LDAP_SEARCH_FILTER, LDAP_ACCOUNT_ATTR,")
                print(
                    "     LDAP_SPECIAL_ACCOUNTS and LDAP_SPECIAL_ACCOUNTS_MASK are also supported."
                )
                sys.exit()

    if not ldapHost or not ldapBindUser or not ldapBindPass or not baseDN:
        print(
            "ERROR: LDAP host, bind user, bind password and base DN are required.",
            file=sys.stderr,
        )
        print(
            "       Provide them as command-line arguments or environment variables.",
            file=sys.stderr,
        )
        sys.exit(1)
