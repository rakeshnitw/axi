  ...
  """
    ...
    try:
        ...
        if logFile != None:
            elog(
                fdlog,
                "Warning: Could not open log file (%s), or none specified" % logFile,
            )
        fdlog = None
    ilog(fdlog, "")
    ...
    ilog(
        fdlog,
        "** Last login date before: %s (%s days)" % (time.ctime(thenDate), noDays),
    )
            ...
            if last_login < thenDate:
                ...
                if action == "list":
                    if account in exclude:
                        ...
                        ilog(
                            fdlog,
                            ">> Exclude protected account %s@%s (Last login: %s)"
                            % (account, domain, time.ctime(last_login)),
                        )
                    else:
                        a_del += 1
                        ilog(
                ...
                if action == "delete":
                    try:
                        if account in exclude or full_account in exclude:
                            ...
                            ilog(
                                fdlog,
                                ">> Success deleting account %s@%s (Last login: %s)"
                                % (account, domain, time.ctime(last_login)),
                            )
                    except:
                        a_del_err += 1
                        elog(
                            fdlog,
                            "!! Error deleting account %s@%s (Last login: %s)"
                            % (account, domain, time.ctime(last_login)),
                        )
    ...
    if action == "list":
        ilog(
            fdlog,
            "** List info: %d domains, %d domain-errors, %d accounts, %d accounts-errors, %d login-time, %d excluded, %d to-delete"
            % (d_tot, d_err, a_tot, a_err, a_time, a_exclude, a_del),
        )
    if action == "delete":
        ilog(
            fdlog,
            "** Delete info: %d domains, %d domain-errors, %d accounts, %d accounts-errors, %d login-time, %d excluded, %d deleted, %d delete-errors"
            % (d_tot, d_err, a_tot, a_err, a_time, a_exclude, a_del, a_del_err),
        )
